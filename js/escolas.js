...
const escolas = {
    ...
    async adicionarEscola() {
        ...
        if (escolasExistentes.length >= 1) {
            utils.mostrarToast('A versão Free permite apenas 1 escola', 'warning');
            return;
        }
        ...
        if (input) {
            ...
            input.placeholder = possuiEscola ? 'A versão Free permite apenas 1 escola' : 'Ex: Colégio São José';
        }
    ...
    async mostrarModalGerenciar() {
        ...
        if (input) {
            ...
            input.placeholder = escolasExistentes.length >= 1 ? 'A versão Free permite apenas 1 escola' : 'Ex: Colégio São José';
        }
